...
public class PrimativeDataType {
    ...
    public static short parity(long x) {
        ...
        while (x != 0) {
            result ^= (x & 1); /*
                                * ^= 는 XOR(배타적 논리합)연산 서로 같으면 0,서로 다르면 1
    ...
    public static long revserBits(long x) { // x는 64비트임
        ...
        return precomputedRevers[(int) (x & BIT_MASK)] << (3 * WORD_SIZE) // 64비트 숫자를 16비트 단위로 분할하여 변환
                                                                          ...
                                                                          // << (3 * WORD_SIZE) = << 48 → 맨 왼쪽(상위 16비트)로
                                                                          // 이동

                | precomputedRevers[(int) ((x >>> WORD_SIZE) & BIT_MASK)] << (2 * WORD_SIZE) // x >>> WORD_SIZE → 16비트
                                                                                             // 오른쪽으로 이동하여 두 번째 16비트를 추출
    ...
    public static int uniformRandom(int lowerBound, int upperBound) {
        ...
        do {
            ...
            for (int i = 0; (1 << i) < numberOfOutcomes; ++i) {
                result = (result << 1) | zeroOneRandom(); // zeroOneRaondom()은 임의의 숫자를 생성하는 함수
            }
